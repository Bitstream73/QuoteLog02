...
async function renderCategory(id) {
  ...
  try {
    const [catData, quotesData] = await Promise.all([
      ...
      API.get(`/categories/${id}/quotes?limit=50`),
    ]);
    ...
    if (topics.length > 0) {
      html += `<div class="category-topics">
        <div class="category-topics-list">
          ${topics.map(t => `<span class="category-topic-chip">${escapeHtml(t.name)}</span>`).join('')}
        </div>
      </div>`;
    }
      ...
      if (quotesData.totalPages > 1) {
        html += `
          <div class="pagination">
            ...
            <button class="page-btn" onclick="loadCategoryQuotesPage('${id}', 2)">Next &rarr;</button>
          </div>
        `;
      }
    }
...
async function loadCategoryQuotesPage(categoryId, page) {
  try {
    const quotesData = await API.get(`/categories/${categoryId}/quotes?page=${page}&limit=50`);
