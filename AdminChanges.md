...
<!-- TODO START -->
Move the Quote management section in settings over to the Review modal
When logged in as an admin, you should be able to edit all the items in a quote and article anywhere on the site - not just in Quote Management
When in admin mode, when there is no image available for an author, make the author's name and description a clickable link that searches images on google using the author's name and description
Add a new tab to the homepage called "Top Stories". Make it the default open tab.
In settings/news sources add a "Top Stories" checkbox that, when clicked, includes the stories from that News Source in the "Top Stories" tab on the homepage.
For articles, when in admin mode, add a checkbox to each article that, when checked, adds the article to the "Top Stories" tab on the homepage.

Move settings/news sources under settings/database
<!-- TODO END -->
