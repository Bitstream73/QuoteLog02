...
export async function insertAndDeduplicateQuote(quoteData, personId, article, db) {
  const { text, quoteType, context, sourceUrl, rssMetadata, quoteDate, isVisible, extractedKeywords, extractedTopics } = quoteData;
  ...
  if (candidates.length === 0) {
    ...
    return insertNewQuote(text, quoteType, context, sourceUrl, personId, article.id, db, rssMetadata, quoteDate, isVisible, extractedKeywords, extractedTopics);
  }
 ...
 * Insert a new quote
 */
function insertNewQuote(text, quoteType, context, sourceUrl, personId, articleId, db, rssMetadata, quoteDate, isVisible, extractedKeywords, extractedTopics) {
  const sourceUrls = JSON.stringify([sourceUrl]);

  const result = db.prepare(`INSERT INTO quotes
    (person_id, text, quote_type, context, source_urls, rss_metadata, quote_datetime, is_visible, extracted_keywords, extracted_topics)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(personId, text, quoteType || 'direct', context, sourceUrls, rssMetadata || null, quoteDate || null, isVisible !== undefined ? isVisible : 1, extractedKeywords || null, extractedTopics || null);
