...
** Implementation **
For each fetch cycle, fetch articles from the past that are not yet ingested. Fetch the same amount as specified in settings/Max Articles per News Source per Cycle. Ideally, the site will eventually have quotes going all the way back to the 18th century.

Go wild on sources. Unlike fetching current articles from RSS feeds, the sources for past quotes can range from news sources, historical archives, wikipedia, wayback machine, API's, RSS's. RESEARCH SOURCES THOROUGHLY, then add those that will provide sourced quotes from public figures on the national stage.

Add a "Historical Sources" section to settings where each of the sources you implement are listed, with the ability to turn each source on or off and an indication of whether or not the source is working as intended.
