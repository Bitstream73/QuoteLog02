# QuoteLog02 Upvote & Analytics — Autonomous Build
...
- `docs/UPVOTE.md` — upvote system spec (schema, API, frontend)
- `docs/ANALYTICS.md` — analytics modal spec (queries, UI, metrics)
- `docs/DATABASE.md` — all schema changes (tables, indexes, migrations)

Check git log for what was done in previous iterations:
- `git log --oneline -20`

**IMPORTANT:** This is a feature addition to an EXISTING app. All new code must integrate with the existing codebase at `E:\Github Repos\QuoteLog02`. Read existing files before modifying them. Match existing patterns exactly.
...
- Database schema: modify `src/config/database.js` (add tables in `initializeDatabase()`)
- New routes: create files in `src/routes/`, mount in `src/index.js`
- Existing routes: modify `src/routes/quotes.js` to include vote counts
- Frontend pages: modify `public/js/home.js`, `quote.js`, `author.js`, `article.js`
- New frontend: create `public/js/analytics.js` and `public/js/vote.js`
- Styles: append to `public/css/styles.css`
- HTML: modify `public/index.html` for header nav + modal container
...
- **Anonymous voting uses voter_hash** — see docs/UPVOTE.md for details.
